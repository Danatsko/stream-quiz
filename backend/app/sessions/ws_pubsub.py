...
async def global_take_redis_pubsub_listener(redis_client: Redis) -> None:
    pubsub = redis_client.pubsub()
    match_pattern = await get_session_take_events_channel(uuid="*")
...
async def global_host_redis_pubsub_listener(redis_client: Redis) -> None:
    pubsub = redis_client.pubsub()
    match_pattern = await get_session_host_events_channel(uuid="*")
    ...
    try:
        async for message in pubsub.listen():
            if message["type"] == "pmessage":
                ...
                session_uuid_str = channel_name.split(":")[2]
