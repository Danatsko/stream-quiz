...
const formatOnBlur = (type: 'h' | 'm' | 's'): void => {
  if (type === 'h' && hours.value) hours.value = hours.value.padStart(2, '0')
  if (type === 'm' && minutes.value) minutes.value = minutes.value.padStart(2, '0')
  if (type === 's' && seconds.value) seconds.value = seconds.value.padStart(2, '0')
}
</script>

<template>
  <div class="duration-input-wrapper" :id="id">
    <input
